    ...
    encryptAll  : function(e, callback) {
        ...
        $$('.form-textbox, .form-textarea, .form-radio, .form-checkbox, .form-dropdown, .form-number-input').each(function(field){
            ...
            //if(JotForm.isVisible(field)) {
                ...
                if (alreadyEncrypted.indexOf(encryptedFieldName) !== -1 && (!field.up().hasClassName('form-matrix-values') && !field.up(1).hasClassName('form-matrix-values'))
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    setQuestionMasking: function (toSelector, type, maskValue, unmask) {
        ...
        if (unmask) {
            ...
            jqObject.mask(maskValue, {placeholder: "_"})
    ...
    initPaymentNew: function(id) {
        var qLine = $$('#id_1')[0];
        var products = qLine.select('.paymentCard-list li');

        var card = qLine.select('.jfCard')[0];
        var cardHeight = card.getBoundingClientRect().height;

        products.each(function(p) {
            p.observe('click', function(item) {
                card.style.height = cardHeight + 'px';

                qLine.select('.jfQuestion-label')[0].hide();
                qLine.select('.jfQuestion-description')[0].hide();

                qLine.select('.js-product-wrapper')[0].hide();
                qLine.select('.details-page')[0].show();

                qLine.select('.jfProduct-row').each(function(item) {
                    $(item).hide();
                });

                var pid = this.getAttribute('data-pid');
                var productDetails = qLine.select('.js-product-row-' + pid)[0];

                productDetails.show();
            });
        });

        var backToProduct = function () {
            card.style.height = null;

            qLine.select('.jfQuestion-label')[0].show();
            qLine.select('.jfQuestion-description')[0].show();

            qLine.select('.js-product-wrapper')[0].show();
            qLine.select('.details-page')[0].hide();
        }

        var backButton = qLine.select('.js-back-to-product')[0];
        backButton.observe('click', function() {
            backToProduct();
        });

        var addToBagButton = qLine.select('.js-add-to-bag')[0];
        addToBagButton.observe('click', function() {
            backToProduct();
        });
    }
};
